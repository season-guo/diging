...
void creatquery(char *domain,uint16_t qtype,uint8_t *query,size_t& querylen){
	//header first
	query[0]=0;
	query[1]=0;
	query[2]=0x01;
	...
	for(char *i=domain;;i++){
		len++;
		if(*i=='.') query[lastlen]=splitlen,splitlen=0,lastlen=len;
		else if(*i=='\0'){
			query[lastlen]=splitlen;
			query[len]=0;
			break;
		}
	        else splitlen+=1,query[len]=*i;
	};
	cout<<len<<endl;
	query[++len]=0;
	query[++len]=1;
...
int main(int argc,char* argv[]){
	...
	if(argv[0][0]='@'){
	        ...
	        domain=argv[1];
		if(argc==3){
			if(argv[2][0]=='A') qtype=A;
			else qtype=NS;
		}
	}else{
	        domain=argv[0];
		if(argc==2){
			if(argv[1][0]=='A') qtype=A;
			else qtype=NS;
		}
	}
	size_t querylen;
